

import tensorflow as tf
from tensorflow.examples.tutorials.mnist import input_data


mnist= input_data.read_data_sets("MNIST_data/", one_hot=True)

session = tf.InteractiveSession()

f = tf.Variable(tf.zeros([10]))
a = tf.placeholder(tf.float32, [None, 784])
b = tf.placeholder(tf.float32, [None, 10])
c = tf.Variable(tf.zeros([784, 10]))

y = tf.nn.softmax(tf.matmul(a, c) + f)

def weight_variable(shape):
  initial = tf.truncated_normal(shape, stddev=0.1)
  return tf.Variable(initial)

def bias_variable(shape):
  initial = tf.constant(0.1, shape=shape)
  return tf.Variable(initial)


def conv2d(x, c):
  return tf.nn.conv2d(x, c, strides=[1, 1, 1, 1], padding='SAME')

def max_pool_2x2(x):
  return tf.nn.max_pool(x, ksize=[1, 2, 2, 1],
                        strides=[1, 2, 2, 1], padding='SAME')


c_conv1 = weight_variable([5, 5, 1, 32])
f_conv1 = bias_variable([32])

x_image = tf.reshape(x, [-1,28,28,1])
h_conv1 = tf.nn.relu(conv2d(x_image, c_conv1) + f_conv1)
h_pool1 = max_pool_2x2(h_conv1)


c_conv2 = weight_variable([5, 5, 32, 64])
f_conv2 = bias_variable([64])

h_conv2 = tf.nn.relu(conv2d(h_pool1, c_conv2) + f_conv2)
h_pool2 = max_pool_2x2(h_conv2)

c_fc1 = weight_variable([7 * 7 * 64, 1024])
f_fc1 = bias_variable([1024])

h_pool2_flat = tf.reshape(h_pool2, [-1, 7*7*64])
h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, c_fc1) + f_fc1)

keep_prob = tf.placeholder(tf.float32)
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

c_fc2 = weight_variable([1024, 10])
f_fc2 = bias_variable([10])

y_conv=tf.nn.softmax(tf.matmul(h_fc1_drop, c_fc2) + f_fc2)


cross_entropy = -tf.reduce_sum(b*tf.log(y_conv))
train_step = tf.train.AdamOptimizer(1e-4).minimize(cross_entropy)
correct_prediction = tf.equal(tf.argmax(y_conv,1), tf.argmax(b,1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))



saver = tf.train.Saver()
session.run(tf.initialize_all_variables())

for i in range(20000):
  batch = mnist.train.next_batch(50)
  if i%100 == 0:
    train_accuracy = accuracy.eval(feed_dict={
        a:batch[0], b: batch[1], keep_prob: 1.0})
    print("step %d, training accuracy %g"%(i, train_accuracy))
  train_step.run(feed_dict={a: batch[0], b: batch[1], keep_prob: 0.5})

save_path = saver.save(session, "created_model.ckpt")
print ("Model saved in file: ", save_path)

print("test accuracy %g"%accuracy.eval(feed_dict={
    a: mnist.test.images, b: mnist.test.labels, keep_prob: 1.0}))
